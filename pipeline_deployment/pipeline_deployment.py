...
def main(CONFIG):
        ...
        for compute_name, num_nodes in compute_config.items():
            try:
                ...
                if compute_name in required_computes:            
                    config = AmlCompute.provisioning_configuration(vm_size=vm_sizes[compute_type],
                                                vm_priority="lowpriority", 
                                                min_nodes=num_nodes["min"],
                                                max_nodes=num_nodes["max"])
                    compute_targets[compute_name] = ComputeTarget.create(workspace=ws, name=compute_name, provisioning_configuration=config)
    ...
    pipeline_args: List[Union[str, PipelineParameter]] = []
    for step in pipeline_params:
